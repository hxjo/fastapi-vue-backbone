...
@router.get("/signup", dependencies=[RedirectIfAlreadyLoggedInDep], include_in_schema=False)
...
@router.get("/login", dependencies=[RedirectIfAlreadyLoggedInDep], include_in_schema=False)
...
async def api_login(
    ...
    try:
        ...
        response.set_cookie(key="token", value=token.access_token, httponly=True, samesite=None, secure=True, expires=datetime.now(timezone.utc) + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES))
        return response
    except InvalidCredentialsException as exc:
        response = RedirectResponse(url=f"/login", status_code=303)
        response.set_cookie("error", exc.message, path="/", samesite=None, secure=True, expires=3)
        return response

@router.get("/logout")
async def logout():
    response = RedirectResponse(url=f"/", status_code=303)
    response.set_cookie("token", "deleted", path="/", expires=datetime.min.replace(tzinfo=timezone.utc), samesite=None, secure=True, httponly=True)
    return response
...
async def send_recover_password_email(email: str, *, session: SessionDep):
    ...
    try:
        ...
        response.set_cookie("message", "auth.recovery_email_sent", path="/", samesite=None, secure=True, expires=3)
...
async def reset_password(body: NewPassword, *, deps: AnnotatedCommonDep):
    ...
    if not user.is_active:
        ...
        response = RedirectResponse(f'/reset-password?token={body.token}', status_code=303)
        response.set_cookie("error", exc.message, path="/", samesite=None, secure=True, expires=3)
        return response
    ...
    response = RedirectResponse(url=f"/app", status_code=303)
    response.set_cookie(key='message', value='auth.password_reset_success', path="/", samesite=None, secure=True, expires=3)
    response.set_cookie(key="token", value=access_token, httponly=True, samesite=None, secure=True,
...
@router.get("/recover-password", dependencies=[RedirectIfAlreadyLoggedInDep], include_in_schema=False)
...
@router.get("/reset-password", dependencies=[RedirectIfAlreadyLoggedInDep], include_in_schema=False)
